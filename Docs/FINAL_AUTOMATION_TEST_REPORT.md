...
This Final Automation Test Report represents the formal culmination of Assignment III (Phase 3: Automation Testing) for the SauceDemo (Swag Labs) e-commerce web application in CST8513. 
...
2. **Phase 2 (Manual Test Execution & Defect Reporting):**  
   ...
   - **Defect D-01 (High Severity):** Empty Cart Checkout Allowed — Users can proceed to checkout with zero products in the cart.
   - **Defect D-02 (Medium Severity):** Weak Customer Information Input Validation — Checkout forms accept single-character inputs without format constraints.
3. **Phase 3 (Automation Framework & Test Implementation - Current Phase):**  
   Successfully developed an automated regression testing suite using **Python 3.11**, **Selenium WebDriver** structured under the **Page Object Model (POM)** design pattern, **Pytest**, and **Robot Framework**. 
...
- **Module 1: Authentication & Authorization** (`FR-01`, `FR-02`, `FR-14` / `TC-01`, `TC-02`, `TC-14`)  
  *Code References:* `automation_selenium/pages/login_page.py`, `automation_selenium/tests/test_authentication.py`
- **Module 2: Product Catalog & Sorting** (`FR-03`, `FR-04`, `FR-05` / `TC-03`, `TC-04`, `TC-05`)  
  *Code References:* `automation_selenium/pages/inventory_page.py`, `automation_selenium/pages/product_details_page.py`, `automation_selenium/tests/test_inventory.py`
- **Module 3: Shopping Cart Operations** (`FR-06`, `FR-07`, `FR-08` / `TC-06`, `TC-07`, `TC-08`)  
  *Code References:* `automation_selenium/pages/cart_page.py`, `automation_selenium/tests/test_cart.py`
- **Module 4: Checkout Process & Mathematical Calculations** (`FR-09`, `FR-10`, `FR-11`, `FR-12`, `FR-13` / `TC-09` to `TC-13`)  
  *Code References:* `automation_selenium/pages/checkout_step_one_page.py`, `automation_selenium/pages/checkout_step_two_page.py`, `automation_selenium/pages/checkout_complete_page.py`, `automation_selenium/tests/test_checkout.py`
- **Module 5: End-to-End User Purchase Lifecycle** (Comprehensive multi-step user journey)  
  *Code Reference:* `automation_selenium/tests/test_e2e_workflow.py`
...
| Test Case ID | Requirement | Feature Area | Automated Test Function & Code Location | Execution Result | Execution Time | Automated Verification Highlights |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TC-01** | FR-01 | Authentication | `test_authentication.py: test_tc01_successful_login` | **PASSED** | 1.42s | Validates redirection to `/inventory.html`, header display, and catalog load. |
| **TC-02-A** | FR-02 | Authentication | `test_authentication.py: test_tc02_login_error_validation[invalid]` | **PASSED** | 1.15s | Asserts "Username and password do not match any user in this service". |
| **TC-02-B** | FR-02 | Authentication | `test_authentication.py: test_tc02_login_error_validation[locked]` | **PASSED** | 1.10s | Asserts "Sorry, this user has been locked out." |
| **TC-02-C** | FR-02 | Authentication | `test_authentication.py: test_tc02_login_error_validation[empty_user]` | **PASSED** | 1.08s | Asserts "Username is required". |
| **TC-02-D** | FR-02 | Authentication | `test_authentication.py: test_tc02_login_error_validation[empty_pass]` | **PASSED** | 1.09s | Asserts "Password is required". |
| **TC-03** | FR-03 | Inventory | `test_inventory.py: test_tc03_inventory_display` | **PASSED** | 1.35s | Verifies exactly 6 products, item titles, prices ($7.99 to $49.99), and images. |
| **TC-04-A** | FR-04 | Sorting | `test_inventory.py: test_tc04_product_sorting[az]` | **PASSED** | 1.62s | Verifies alphabetical ordering (A to Z) on catalog. |
| **TC-04-B** | FR-04 | Sorting | `test_inventory.py: test_tc04_product_sorting[za]` | **PASSED** | 1.58s | Verifies reverse alphabetical ordering (Z to A) on catalog. |
| **TC-04-C** | FR-04 | Sorting | `test_inventory.py: test_tc04_product_sorting[lohi]` | **PASSED** | 1.60s | Verifies ascending price ordering ($7.99 -> $49.99). |
| **TC-04-D** | FR-04 | Sorting | `test_inventory.py: test_tc04_product_sorting[hilo]` | **PASSED** | 1.61s | Verifies descending price ordering ($49.99 -> $7.99). |
| **TC-05** | FR-05 | Product Details | `test_inventory.py: test_tc05_product_details_view` | **PASSED** | 1.48s | Validates item title, description, $29.99 price, image, and back navigation. |
| **TC-06** | FR-06 | Shopping Cart | `test_cart.py: test_tc06_add_to_cart` | **PASSED** | 1.75s | Validates live cart badge incrementing from 0 -> 1 -> 2. |
| **TC-07** | FR-07 | Shopping Cart | `test_cart.py: test_tc07_remove_from_cart` | **PASSED** | 1.82s | Validates live cart badge decrementing and button label state toggle. |
| **TC-08** | FR-08 | Cart Review | `test_cart.py: test_tc08_cart_review` | **PASSED** | 1.55s | Verifies `/cart.html` item list, prices, and continue shopping navigation. |
| **TC-09** | FR-09 | Checkout Info | `test_checkout.py: test_tc09_checkout_empty_fields_validation` | **PASSED** | 1.38s | Submits blank form; asserts "Error: First Name is required". |
| **TC-10-A** | FR-10 | Input Rules | `test_checkout.py: test_tc10_checkout_input_validation[last_name]`| **PASSED** | 1.41s | Missing last name; asserts "Error: Last Name is required". |
| **TC-10-B** | FR-10 | Input Rules | `test_checkout.py: test_tc10_checkout_input_validation[postal_code]`| **PASSED** | 1.40s | Missing postal code; asserts "Error: Postal Code is required". |
| **TC-11** | FR-11 | Order Overview | `test_checkout.py: test_tc11_order_overview_calculations` | **PASSED** | 1.70s | Programmatic formula check: Subtotal ($45.98) + Tax ($3.68) == Total ($49.66). |
| **TC-12** | FR-12 | Order Finish | `test_checkout.py: test_tc12_finish_order_confirmation` | **PASSED** | 1.65s | Validates "Thank you for your order!" confirmation and pony express image. |
| **TC-13** | FR-13 | Checkout Cancel | `test_checkout.py: test_tc13_cancel_checkout_redirection` | **PASSED** | 1.52s | Step 1 cancel returns to Cart; Step 2 cancel returns to Inventory. |
| **TC-14** | FR-14 | User Logout | `test_authentication.py: test_tc14_secure_logout` | **PASSED** | 1.88s | Validates sidebar logout, session termination, and unauthorized back-navigation block. |
| **E2E** | All | E-Commerce | `test_e2e_workflow.py: test_complete_e2e_shopping_workflow` | **PASSED** | 3.45s | Full 13-step purchase journey from login to order confirmation to logout. |
...
- **Automated Handling:** Automated test scripts in `automation_selenium/tests/test_cart.py` and `automation_selenium/tests/test_checkout.py` assert cart state constraints (`assert cart_page.get_cart_item_count() > 0`). In standard regression, this defect scenario is isolated and verified programmatically.
...
- **Automated Handling:** Parameterized automated test cases in `automation_selenium/tests/test_checkout.py` (`test_tc10_checkout_input_validation`) systematically assert required field boundaries and document the application's permissive validation logic.
...
### 8.2 Technical References & Artifacts
1. **Course Syllabus & Instructions:** CST8513 Assignment III Specifications, Prof. Sharmista Datta, Algonquin College.
2. **Phase 1 Project Proposal:** CST8513 Project 1 Requirements Analysis, Devangbhai Pandit & Meet Ahalpara.
3. **Phase 2 Test Cases & Defect Report:** CST8513 Project 2 Test Cases & Defects Report, Devangbhai Pandit & Meet Ahalpara.
4. **Selenium WebDriver Python Documentation:** https://www.selenium.dev/documentation/webdriver/
5. **Pytest Framework Documentation:** https://docs.pytest.org/
6. **Robot Framework Documentation:** https://robotframework.org/
7. **Project GitHub Source Repository:** https://github.com/MeetAhalpara/QA-Test-Automation
